...
func TestExecInInstance(t *testing.T) {
	...
	// Create a test service with 1 instance
	service, instances := createTestServiceWithInstances(ctx, t, testStore, instController, "test-service", 1)
...
func TestExecInInstanceWrongService(t *testing.T) {
	...
	// Create two test services with one instance each
	_, instances1 := createTestServiceWithInstances(ctx, t, testStore, instController, "test-service-1", 1)
	service2, _ := createTestServiceWithInstances(ctx, t, testStore, instController, "test-service-2", 1)
	instance1 := instances1[0]
	...
	assert.Contains(t, err.Error(), "instance does not belong to service", "Error message should mention ownership")
}
